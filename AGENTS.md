...
- **한국어 입력 UI 규칙** — Enter 키 핸들러에는 반드시 `!e.nativeEvent.isComposing` 가드 (한글 IME 조합 확정과 겹침 방지, WEB-6 파일럿에서 확인된 규칙). 또한 controlled input의 조합 가드는 **검색·제출 발사만** 막는다 — onChange에서 `value` 상태 갱신까지 건너뛰면 React가 입력값을 되돌려 IME 입력 자체가 먹통이 된다. 표시값 상태와 발사값 상태를 분리할 것 (WEB-53 검수에서 확인된 실제 버그 — /records·/search 동시 발생).
